/* =========================================================
   utils.c
   -------
   Small helper functions used everywhere in the game.
   Random numbers, grid/pixel conversion, messages,
...
void spawn_particles(float x, float y, Color c, int n) {
    for (int k = 0; k < n; k++) {
        for (int i = 0; i < MAX_PARTICLES; i++) {
            if (G.particles[i].life <= 0.f) {
                float ang = randf() * 6.283f;
                float spd = 40.f + randf() * 80.f;
                G.particles[i] = (Particle){
                    x, y,
                    cosf(ang)*spd, sinf(ang)*spd - 30.f,
                    0.5f + randf()*0.4f, 1.f,
                    c, 3.f + randf()*4.f
                };
                break;
